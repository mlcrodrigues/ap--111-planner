...
const RoomDetail: React.FC<RoomDetailProps> = ({ room, onUpdate, onRemove }) => {
  ...
  const handleAddRepair = (newItem: RepairItem) => {
    onUpdate(room.id, 'repairs', [...room.repairs, newItem]);
  };

  const handleAddMaterial = (newItem: MaterialItem) => {
    onUpdate(room.id, 'materials', [...room.materials, newItem]);
  };

  const handleAddLabor = (newItem: LaborItem) => {
    onUpdate(room.id, 'labor', [...room.labor, newItem]);
  };

  const handleRemoveRepair = (itemId: string) => {
    onUpdate(room.id, 'repairs', room.repairs.filter(item => item.id !== itemId));
  };

  const handleRemoveMaterial = (itemId: string) => {
    onUpdate(room.id, 'materials', room.materials.filter(item => item.id !== itemId));
  };

  const handleRemoveLabor = (itemId: string) => {
    onUpdate(room.id, 'labor', room.labor.filter(item => item.id !== itemId));
  };
            ...
            <div className="space-y-3">
              {room.repairs.map(item => (
                <div key={item.id} className="flex justify-between items-center card" style={{ padding: '0.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  ...
                  <button onClick={() => handleRemoveRepair(item.id)} className="icon-btn-remove" style={{ padding: '0.25rem' }}><TrashIcon style={{ width: '1rem', height: '1rem' }} /></button>
                </div>
              ))}
            </div>
            <form onSubmit={e => { e.preventDefault(); handleAddRepair({ id: Date.now().toString(), description: newRepair, completed: false }); setNewRepair(''); }} className="flex gap-2" style={{ marginTop: '1rem' }}>
              ...
              {room.materials.map(item => (
                <div key={item.id} className="flex justify-between items-center card" style={{ padding: '0.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleRemoveMaterial(item.id)} className="icon-btn-remove" style={{ padding: '0.25rem' }}><TrashIcon style={{ width: '1rem', height: '1rem' }} /></button>
                  </div>
                </div>
              ))}
            </div>
            <form onSubmit={e => { e.preventDefault(); handleAddMaterial({ id: Date.now().toString(), ...newMaterial }); setNewMaterial({ name: '', quantity: 1, unitPrice: 0 }); }} className="flex flex-wrap gap-2" style={{ marginTop: '1rem' }}>
              ...
              {room.labor.map(item => (
                <div key={item.id} className="flex justify-between items-center card" style={{ padding: '0.5rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleRemoveLabor(item.id)} className="icon-btn-remove" style={{ padding: '0.25rem' }}><TrashIcon style={{ width: '1rem', height: '1rem' }} /></button>
                  </div>
                </div>
              ))}
            </div>
            <form onSubmit={e => { e.preventDefault(); handleAddLabor({ id: Date.now().toString(), ...newLabor }); setNewLabor({ providerName: '', phone: '', price: 0 }); }} className="space-y-2" style={{ marginTop: '1rem' }}>
